...
namespace Example {
    void Showroom::init() {
        ...
        float gS = 120.0f;
        gVerts.push_back({ {-gS, -0.01f, -gS}, gColor }); gVerts.push_back({ { gS, -0.01f, -gS}, gColor });
        gVerts.push_back({ { gS, -0.01f,  gS}, gColor }); gVerts.push_back({ {-gS, -0.01f, -gS}, gColor });
        gVerts.push_back({ { gS, -0.01f,  gS}, gColor }); gVerts.push_back({ {-gS, -0.01f,  gS}, gColor });
        ...
        float hCeil = 15.0f;
        ceilVerts.push_back({ {-gS, hCeil, -gS}, ceilColor }); ceilVerts.push_back({ { gS, hCeil, -gS}, ceilColor });
        ceilVerts.push_back({ { gS, hCeil,  gS}, ceilColor }); ceilVerts.push_back({ {-gS, hCeil, -gS}, ceilColor });
        ceilVerts.push_back({ { gS, hCeil,  gS}, ceilColor }); ceilVerts.push_back({ {-gS, hCeil,  gS}, ceilColor });
        ...
        auto addCube = [&](glm::vec3 p, float w, float h, float d, glm::vec3 c) {
            // وجه أمامي
            deskV.push_back({ {p.x - w, p.y, p.z + d}, c }); deskV.push_back({ {p.x + w, p.y, p.z + d}, c }); deskV.push_back({ {p.x + w, p.y + h, p.z + d}, c });
            deskV.push_back({ {p.x - w, p.y, p.z + d}, c }); deskV.push_back({ {p.x + w, p.y + h, p.z + d}, c }); deskV.push_back({ {p.x - w, p.y + h, p.z + d}, c });
            // سطح المكتب
            deskV.push_back({ {p.x - w, p.y + h, p.z - d}, topCol }); deskV.push_back({ {p.x + w, p.y + h, p.z - d}, topCol }); deskV.push_back({ {p.x + w, p.y + h, p.z + d}, topCol });
            deskV.push_back({ {p.x - w, p.y + h, p.z - d}, topCol }); deskV.push_back({ {p.x + w, p.y + h, p.z + d}, topCol }); deskV.push_back({ {p.x - w, p.y + h, p.z + d}, topCol });
            };
        ...
        auto addColumn = [&](float x, float z) {
            ...
            colVerts.push_back({ {x - cw, 0, z + cw}, colCol }); colVerts.push_back({ {x + cw, 0, z + cw}, colCol }); colVerts.push_back({ {x + cw, hCeil, z + cw}, colCol });
            colVerts.push_back({ {x - cw, 0, z + cw}, colCol }); colVerts.push_back({ {x + cw, hCeil, z + cw}, colCol }); colVerts.push_back({ {x - cw, hCeil, z + cw}, colCol });
            // (نكتفي بوجه واحد مواجه للمركز للتبسيط حالياً)
            };
        addColumn(15, 15);   addColumn(-15, 15);
        addColumn(15, -15);  addColumn(-15, -15);
    ...
    glm::vec3 Showroom::checkCollision(glm::vec3 currentPos, glm::vec3 nextPos) {
        ...
        for (auto& room : rooms) {
                ...
                nextPos.z > zMin - margin && nextPos.z < zMax + margin)
            {
                ...
                if (nextPos.x < xMin + margin) nextPos.x = xMin + margin;
                if (nextPos.x > xMax - margin) nextPos.x = xMax - margin;

                // تحديد أي جدار هو الباب
                if (room->doorAtMaxZ) { // الباب عند zMax (الغرف الخلفية)
                    if (nextPos.z < zMin + margin) nextPos.z = zMin + margin; // جدار خلفي مصمت
                    if (nextPos.x < dL || nextPos.x > dR) { // خارج عرض الباب
                        if (currentPos.z < zMax) { if (nextPos.z > zMax - margin) nextPos.z = zMax - margin; }
                        else { if (nextPos.z < zMax + margin) nextPos.z = zMax + margin; }
                    }
                }
                else { // الباب عند zMin (الغرف الأمامية)
                    if (nextPos.z > zMax - margin) nextPos.z = zMax - margin; // جدار أمامي مصمت (بالنسبة للغرفة)
                    if (nextPos.x < dL || nextPos.x > dR) { // خارج عرض الباب
                        if (currentPos.z > zMin) { if (nextPos.z < zMin + margin) nextPos.z = zMin + margin; }
                        else { if (nextPos.z > zMin - margin) nextPos.z = zMin - margin; }
                    }
                }
            }
        }
